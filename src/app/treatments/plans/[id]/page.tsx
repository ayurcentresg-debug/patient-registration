...
const ITEM_STATUS_COLORS: Record<string, { bg: string; color: string; label: string }> = {
  ...
  skipped: { bg: "#fffbeb", color: "#d97706", label: "Skipped" },
};
...
export default function TreatmentPlanDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const updateStatus = async (newStatus: string) => {
    ...
    try {
      ...
      setPlan(updated);
      setToast({ message: `Plan ${newStatus === "active" ? "resumed" : newStatus}`, type: "success" });
    } catch (e: unknown) {
  ...
  const markMilestoneAchieved = async (milestoneId: string) => {
    ...
    try {
      ...
      fetchPlan();
      setToast({ message: "Milestone marked as achieved", type: "success" });
    } catch (e: unknown) {
  ...
  if (!plan) {
    return (
      <div className="p-6 md:p-8">
        <Link href="/treatments/plans" className="inline-flex items-center gap-1.5 text-[13px] font-semibold mb-6" style={{ color: "var(--blue-500)" }}>
  ...
  return (
    <div className="p-6 md:p-8 yoda-fade-in">
      ...
      {/* Back button */}
      <Link href="/treatments/plans" className="inline-flex items-center gap-1.5 text-[13px] font-semibold mb-5" style={{ color: "var(--blue-500)", textDecoration: "none" }}>
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
        Back to Plans
      </Link>
        ...
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 text-[13px] mb-2 flex-wrap" style={{ color: "var(--grey-600)" }}>
              <Link href={`/patients/${plan.patient.id}`} className="font-semibold hover:underline" style={{ color: "var(--blue-500)", textDecoration: "none" }}>
                {patientName}
              </Link>
              <span>&middot;</span>
            ...
            {plan.diagnosis && (
              <span className="inline-flex px-2 py-0.5 text-[10px] font-semibold rounded" style={{ background: "var(--purple-light)", color: "var(--purple)", borderRadius: "var(--radius-sm)" }}>
                {plan.diagnosis}
              </span>
            )}
          </div>
            ...
            <div className="flex items-center gap-2 flex-shrink-0 flex-wrap">
              {plan.status === "active" && (
                <button
                  ...
                  style={{ background: "#fffbeb", color: "#d97706", border: "1px solid #fcd34d", borderRadius: "var(--radius-sm)", opacity: actionLoading ? 0.6 : 1 }}
                >
              ...
              {plan.status === "active" && (
                <button
                  ...
                  disabled={actionLoading !== null}
                  className="inline-flex items-center gap-1.5 px-3.5 py-1.5 text-[12px] font-semibold rounded transition-opacity"
                  style={{ background: "#eff6ff", color: "#3b82f6", border: "1px solid #93c5fd", borderRadius: "var(--radius-sm)", opacity: actionLoading ? 0.6 : 1 }}
                >
            ...
            <div className="relative flex-shrink-0" style={{ width: 80, height: 80 }}>
              <svg width="80" height="80" viewBox="0 0 80 80">
                ...
                <circle
                  ...
                  stroke={pct === 100 ? "#059669" : "var(--blue-500)"}
                  strokeWidth="8"
                  strokeLinecap="round"
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <div className="flex-1 h-1.5 rounded-full overflow-hidden" style={{ background: "var(--grey-200)", minWidth: 60, maxWidth: 80 }}>
                        <div className="h-full rounded-full" style={{ width: `${itemPct}%`, background: itemPct === 100 ? "#059669" : "var(--blue-500)", transition: "width 0.3s ease" }} />
                      </div>
                  ...
                  <td className="px-4 py-3">
                    {canRecord && (
                      <button
                        ...
                        style={{ background: "var(--blue-500)", color: "#fff", border: "none", borderRadius: "var(--radius-sm)", cursor: "pointer", opacity: actionLoading ? 0.6 : 1 }}
                      >
