  ...
  return (
    <div className="p-6 md:p-8 yoda-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <Link href="/treatments/plans/new" className="inline-flex items-center gap-2 text-white px-5 py-2 text-[13px] font-semibold rounded" style={{ background: "var(--blue-500)", borderRadius: "var(--radius-sm)" }}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        {[
          { label: "Active Plans", value: activePlans, color: "var(--green)", bg: "#ecfdf5" },
          { label: "Completed", value: completedPlans, color: "#3b82f6", bg: "#eff6ff" },
          { label: "Paused", value: pausedPlans, color: "#d97706", bg: "#fffbeb" },
          { label: "Total Sessions Done", value: totalSessionsDone, color: "var(--blue-500)", bg: "var(--blue-50)" },
        ].map(s => (
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {plans.map(plan => {
            ...
            return (
              <Link key={plan.id} href={`/treatments/plans/${plan.id}`} className="block" style={{ textDecoration: "none" }}>
                <div className="p-4 transition-shadow hover:shadow-md" style={cardStyle}>
                  <div className="flex flex-col md:flex-row md:items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {plan.diagnosis && (
                        <span className="inline-flex px-2 py-0.5 text-[10px] font-semibold rounded mb-2" style={{ background: "var(--purple-light)", color: "var(--purple)", borderRadius: "var(--radius-sm)" }}>
                          {plan.diagnosis}
                        </span>
                      )}
                      {/* Progress bar */}
                      <div className="flex items-center gap-3 mt-1">
                        <div className="flex-1 h-2 rounded-full overflow-hidden" style={{ background: "var(--grey-200)", maxWidth: 200 }}>
                          <div className="h-full rounded-full transition-all" style={{ width: `${pct}%`, background: pct === 100 ? "#059669" : "var(--blue-500)" }} />
                        </div>
