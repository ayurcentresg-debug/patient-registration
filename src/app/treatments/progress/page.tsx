...
export default function ProgressTrackerPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setToast({ message: "Failed to load progress data", type: "error" });
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  return (
    <div className="p-6 md:p-8 yoda-fade-in">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
          {[
            ...
            { label: "Active Plans", value: stats.activePlans, color: "var(--green)", bg: "#ecfdf5" },
            { label: "Completed", value: stats.completedPlans, color: "#3b82f6", bg: "#eff6ff" },
            { label: "Overall Progress", value: `${stats.overallProgress}%`, color: "#7c3aed", bg: "#f5f3ff" },
            { label: "Sessions Done", value: stats.completedSessions, color: "var(--blue-500)", bg: "var(--blue-50)" },
            { label: "Upcoming Milestones", value: stats.upcomingMilestones?.length ?? 0, color: "#d97706", bg: "#fffbeb" },
          ].map(s => (
      ...
      {stats && !loading && stats.totalSessions > 0 && (
        <div className="mb-8 p-5" style={cardStyle}>
          ...
          <div className="w-full h-4 rounded-full overflow-hidden" style={{ background: "var(--grey-200)" }}>
            <div className="h-full rounded-full transition-all duration-700" style={{ width: `${stats.overallProgress}%`, background: "linear-gradient(90deg, #7c3aed, #a78bfa)" }} />
          </div>
          {/* Mini bar chart: breakdown by status */}
          <div className="flex items-center gap-4 mt-3">
            {[
              { label: "Active", count: stats.activePlans, color: "#059669" },
              { label: "Completed", count: stats.completedPlans, color: "#3b82f6" },
              { label: "Paused", count: stats.pausedPlans, color: "#d97706" },
            ].map(b => (
                ...
                <div key={plan.id} className="p-5" style={cardStyle}>
                  ...
                  <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3 mb-4">
                    ...
                    <div className="flex items-center gap-3 flex-shrink-0">
                      ...
                      <Link href={`/treatments/plans/${plan.id}`} className="inline-flex items-center gap-1 px-3 py-1.5 text-[12px] font-semibold rounded" style={{ background: "var(--blue-50)", color: "var(--blue-500)", borderRadius: "var(--radius-sm)", textDecoration: "none" }}>
                        View Plan
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    <div className="flex-1 h-3 rounded-full overflow-hidden" style={{ background: "var(--grey-200)" }}>
                      <div className="h-full rounded-full transition-all duration-500" style={{ width: `${pct}%`, background: pct === 100 ? "#059669" : "linear-gradient(90deg, var(--blue-500), #60a5fa)" }} />
                    </div>
                  ...
                  {plan.items && plan.items.length > 0 && (
                    <div className="space-y-2 p-3 rounded-lg" style={{ background: "var(--grey-50)", border: "1px solid var(--grey-200)", borderRadius: "var(--radius-sm)" }}>
                      ...
                      {plan.items.map(item => {
                        ...
                        const itemStatusColor = item.status === "completed" ? "#059669" : item.status === "in_progress" ? "var(--blue-500)" : "var(--grey-500)";
                        return (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div>
          <h2 className="text-[16px] font-bold mb-4" style={{ color: "var(--grey-900)" }}>
            <svg className="w-5 h-5 inline-block mr-2 -mt-0.5" style={{ color: "#d97706" }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" /></svg>
            Upcoming Milestones
          </h2>
          {loading ? (
        ...
        <div>
          <h2 className="text-[16px] font-bold mb-4" style={{ color: "var(--grey-900)" }}>
            <svg className="w-5 h-5 inline-block mr-2 -mt-0.5" style={{ color: "#3b82f6" }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
            Recent Activity
          </h2>
          {loading ? (
                  ...
                  <div key={`${item.id}-${idx}`} className="p-4 flex items-center gap-3" style={cardStyle}>
                    <div className="w-8 h-8 flex items-center justify-center rounded-full flex-shrink-0" style={{ background: item.completedSessions === item.totalSessions ? "#ecfdf5" : "#eff6ff" }}>
                      {item.completedSessions === item.totalSessions ? (
                        ...
                        <svg className="w-4 h-4" style={{ color: "#3b82f6" }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" /></svg>
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                    ...
                    <div className="text-right flex-shrink-0">
                      <p className="text-[12px] font-bold" style={{ color: item.completedSessions === item.totalSessions ? "#059669" : "var(--blue-500)" }}>
            ...
            <div className="relative" style={{ width: 120, height: 120 }}>
              <div style={{
                ...
                background: (() => {
                  ...
                  return `conic-gradient(#059669 0deg ${activeAngle}deg, #3b82f6 ${activeAngle}deg ${activeAngle + completedAngle}deg, #d97706 ${activeAngle + completedAngle}deg ${activeAngle + completedAngle + pausedAngle}deg, #94a3b8 ${activeAngle + completedAngle + pausedAngle}deg ${activeAngle + completedAngle + pausedAngle + otherAngle}deg)`;
                })(),
              }} />
