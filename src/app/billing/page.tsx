...
export default function BillingPage() {
  ...
  const fetchInvoices = useCallback(() => {
    ...
    fetch(`/api/invoices?${params}`)
      ...
      .then((data) => {
        ...
        const mapped: Invoice[] = list.map((inv: Record<string, unknown>) => ({
          id: inv.id as string,
  ...
  useEffect(() => { setPage(1); }, [search, statusFilter, branchFilter, dateFrom, dateTo]);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-[15px] mt-0.5" style={{ color: "var(--grey-600)" }}>
            {totalCount > 0 ? `${totalCount} invoice${totalCount !== 1 ? "s" : ""} found` : `${sorted.length} invoice${sorted.length !== 1 ? "s" : ""}`}
          </p>
        </div>
      ...
      <div className="mb-5 space-y-3">
        <div className="flex flex-col sm:flex-row gap-3 flex-wrap">
          ...
          <select
            ...
            className="w-full sm:w-auto px-3 py-2 text-[15px]"
            ...
            <div className="flex items-center justify-between mt-4 px-2">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  disabled={page <= 1}
                  className="px-3 py-1.5 text-[13px] font-semibold rounded-md border transition-colors disabled:opacity-40"
                  style={{ borderColor: "var(--grey-300)", color: "var(--grey-700)", background: "var(--white)" }}
                >
                  Previous
                </button>
                {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                  ...
                  return (
                    <button
                      key={p}
                      onClick={() => setPage(p)}
                      className="w-8 h-8 text-[13px] font-semibold rounded-md transition-colors"
                      style={{
                        background: p === page ? "var(--blue-500)" : "var(--white)",
                        color: p === page ? "white" : "var(--grey-700)",
                        border: p === page ? "none" : "1px solid var(--grey-300)",
                      }}
                    >
                      {p}
                    </button>
