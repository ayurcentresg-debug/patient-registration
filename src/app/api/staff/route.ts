...
export async function POST(request: NextRequest) {
  try {
    ...
    const { name, email, phone, role, gender, dateOfBirth, ethnicity, residencyStatus, prStartDate, specialization, department, branchId, branchRestricted, branchSchedules, consultationFee, schedule, slotDuration, status, dateOfJoining, nricFin, jobTitle, mainDuties, employmentType, isWorkman, weeklyContractedHours, workingDaysPerWeek, sendInvite } = body;
    ...
    const user = await db.user.create({
      data: {
        ...
        branchRestricted: !!branchRestricted && !!branchId,
        branchSchedules: branchSchedules || null,
