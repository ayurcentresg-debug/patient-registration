...
export async function POST() {
  ...
  try {
    ...
    // Forward links — reciprocals are auto-generated by createReciprocalLink below
    const forwardLinks = [
    ...
    for (const fl of forwardLinks) {
      const fwd = await prisma.familyMember.create({ data: fl });
      await createReciprocalLink(prisma, fwd);
    }
