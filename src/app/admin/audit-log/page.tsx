...
export default function AuditLogPage() {
  ...
  return (
    <div className="p-6 md:p-8 yoda-fade-in">
      ...
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-[22px] font-bold tracking-tight" style={{ color: "var(--grey-900)" }}>Audit Log</h1>
      ...
      <div className="flex flex-col gap-3 mb-5">
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Search audit entries..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2 text-[13px]" style={inputStyle} />
          </div>
          <select value={actionFilter} onChange={e => setActionFilter(e.target.value)} className="px-3 py-2 text-[13px]" style={{ ...inputStyle, minWidth: 140 }}>
            <option value="all">All Actions</option>
            <option value="create">Create</option>
            <option value="update">Update</option>
            <option value="delete">Delete</option>
          </select>
          <select value={entityFilter} onChange={e => setEntityFilter(e.target.value)} className="px-3 py-2 text-[13px]" style={{ ...inputStyle, minWidth: 150 }}>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <input type="text" placeholder="Filter by user..." value={userFilter} onChange={e => setUserFilter(e.target.value)} className="px-3 py-2 text-[13px]" style={{ ...inputStyle, minWidth: 180 }} />
          <div className="flex items-center gap-2">
            <span className="text-[12px] font-semibold" style={{ color: "var(--grey-600)" }}>From:</span>
            <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)} className="px-3 py-2 text-[13px]" style={inputStyle} />
          </div>
          <div className="flex items-center gap-2">
            <span className="text-[12px] font-semibold" style={{ color: "var(--grey-600)" }}>To:</span>
            <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)} className="px-3 py-2 text-[13px]" style={inputStyle} />
          </div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-[14px] font-semibold" style={{ color: "var(--grey-700)" }}>No audit entries found</p>
          <p className="text-[12px] mt-1" style={{ color: "var(--grey-500)" }}>Adjust your filters or check back later</p>
        </div>
      ) : (
            ...
            return (
              <div key={entry.id} className="p-4 transition-shadow hover:shadow-sm" style={cardStyle}>
                <div className="flex flex-col md:flex-row md:items-center gap-2">
                  {/* Timestamp */}
                  <span className="text-[11px] font-mono flex-shrink-0 w-40" style={{ color: "var(--grey-500)" }}>
                    {formatTimestamp(entry.timestamp)}
                  </span>

                  {/* User */}
                  <span className="text-[12px] font-semibold flex-shrink-0 w-32" style={{ color: "var(--grey-800)" }}>
                    {entry.userName}
                  </span>

                  {/* Action badge */}
                  <span className="inline-flex px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide rounded flex-shrink-0" style={{ background: ac.bg, color: ac.color, borderRadius: "var(--radius-sm)" }}>
                    {ac.label}
                  </span>

                  {/* Entity */}
                  <span className="text-[12px] flex-1 min-w-0" style={{ color: "var(--grey-700)" }}>
                    <span className="font-semibold capitalize">{entry.entityType}</span>
                    <span className="ml-1 font-mono text-[11px]" style={{ color: "var(--grey-500)" }}>#{entry.entityId.slice(0, 8)}</span>
                  </span>

                  {/* IP & expand */}
                  <div className="flex items-center gap-2 flex-shrink-0">
                    {entry.ipAddress && (
                      <span className="text-[10px] font-mono px-2 py-0.5 rounded" style={{ background: "var(--grey-100)", color: "var(--grey-500)", borderRadius: "var(--radius-sm)" }}>
                        {entry.ipAddress}
                      </span>
                    )}
                    {entry.details && (
                ...
                {isExpanded && entry.details && (
                  <div className="mt-3 pt-3" style={{ borderTop: "1px solid var(--grey-200)" }}>
                    <pre className="text-[11px] p-3 rounded overflow-x-auto whitespace-pre-wrap" style={{ background: "var(--grey-50)", color: "var(--grey-700)", borderRadius: "var(--radius-sm)", fontFamily: "monospace" }}>
                      {entry.details}
                    </pre>
                  </div>
                )}
        ...
        <div className="flex items-center justify-between mt-6 pt-4" style={{ borderTop: "1px solid var(--grey-200)" }}>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={page === 1}
              className="px-3 py-1.5 text-[12px] font-semibold rounded disabled:opacity-40"
            ...
            {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
              ...
              return (
                <button
                  ...
                  className="w-8 h-8 text-[12px] font-semibold rounded"
                  style={{
