          ...
          <table className="w-full text-[14px]">
            <thead>
              <tr style={{ background: "var(--grey-50)" }}>
                ...
                <th className="text-right px-4 py-3 font-semibold" style={{ color: "var(--grey-600)" }}>CPF</th>
            ...
            <tbody>
              {payrolls.map((p) => {
                ...
                return (
                  <tr key={p.id} style={{ borderBottom: "1px solid var(--grey-200)" }}>
                    ...
                    <td className="px-4 py-3 text-right tabular-nums">{formatCurrency(p.totalAllowances)}</td>
                    <td className="px-4 py-3 text-right tabular-nums">{formatCurrency(p.commission)}</td>
                    ...
                    <td className="px-4 py-3 text-right tabular-nums font-semibold">{formatCurrency(p.grossPay)}</td>
                    <td className="px-4 py-3 text-right tabular-nums">{formatCurrency(p.cpfEmployee)}</td>
                    <td className="px-4 py-3 text-right tabular-nums">{formatCurrency(p.totalDeductions)}</td>
              ...
              <tr style={{ borderTop: "2px solid var(--grey-400)", background: "var(--grey-50)" }}>
                <td className="px-4 py-3 font-bold" style={{ color: "var(--grey-900)" }} colSpan={2}>
                  TOTALS
                </td>
                ...
                <div key={config.id} style={cardStyle} className="p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-6 text-[14px]">
                      ...
                      <div className="text-right">
                        <div style={{ color: "var(--grey-500)" }} className="text-[12px]">CPF (E/R)</div>
                        <div className="font-semibold" style={{ color: "var(--grey-700)" }}>{config.cpfEmployee}% / {config.cpfEmployer}%</div>
                      </div>
...
function ReportsTab() {
  ...
  // Monthly summaries
  const monthlyMap = new Map<string, { gross: number; cpfEmployee: number; cpfEmployer: number; net: number; count: number }>();
  for (const p of payrolls) {
    const existing = monthlyMap.get(p.period) || { gross: 0, cpfEmployee: 0, cpfEmployer: 0, net: 0, count: 0 };
  ...
  // Per-staff YTD
  const staffMap = new Map<string, { name: string; role: string; gross: number; cpfEmployee: number; cpfEmployer: number; net: number }>();
  for (const p of payrolls) {
    const existing = staffMap.get(p.userId) || { name: p.userName, role: p.userRole, gross: 0, cpfEmployee: 0, cpfEmployer: 0, net: 0 };
    existing.gross += p.grossPay;
    existing.cpfEmployee += p.cpfEmployee;
    existing.cpfEmployer += p.cpfEmployer;
    existing.net += p.netPay;
  ...
  // CSV export
  const exportCSV = () => {
    const headers = ["Period", "Staff Name", "Role", "Base Salary", "Allowances", "Commission", "Overtime", "Bonus", "Gross Pay", "CPF Employee", "CPF Employer", "Deductions", "Net Pay", "Status"];
    const rows = payrolls.map((p) => [
      p.period, p.userName, p.userRole, p.baseSalary, p.totalAllowances, p.commission,
      p.overtime, p.bonus, p.grossPay, p.cpfEmployee, p.cpfEmployer, p.totalDeductions, p.netPay, p.status,
    ]);
  ...
  // CPF report CSV
  const exportCPF = () => {
    const headers = ["Staff Name", "Employee CPF", "Employer CPF", "Total CPF"];
    const rows = staffSummary.map(([, s]) => [
      s.name, s.cpfEmployee.toFixed(2), s.cpfEmployer.toFixed(2), (s.cpfEmployee + s.cpfEmployer).toFixed(2),
    ]);
    const csv = [headers.join(","), ...rows.map((r) => r.join(","))].join("\n");
    const blob = new Blob([csv], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `cpf-report-${fromPeriod}-to-${toPeriod}.csv`;
    a.click();
    URL.revokeObjectURL(url);
  };
    ...
    <div>
      ...
      {loading ? (
        ...
        <>
            ...
            <table className="w-full text-[14px]">
              <thead>
                <tr style={{ background: "var(--grey-50)" }}>
                  ...
                  <th className="text-right px-4 py-3 font-semibold" style={{ color: "var(--grey-600)" }}>CPF (Employee)</th>
                  <th className="text-right px-4 py-3 font-semibold" style={{ color: "var(--grey-600)" }}>CPF (Employer)</th>
              ...
              <tbody>
                {monthlySummary.map(([period, data]) => (
                  <tr key={period} style={{ borderBottom: "1px solid var(--grey-200)" }}>
                    ...
                    <td className="px-4 py-3 text-right tabular-nums">{formatCurrency(data.cpfEmployer)}</td>
                    <td className="px-4 py-3 text-right tabular-nums font-bold" style={{ color: "var(--blue-500)" }}>
          ...
          {/* CPF Report */}
          <h2 className="text-lg font-bold mb-3" style={{ color: "var(--grey-900)" }}>CPF Contribution Report</h2>
          <div style={cardStyle} className="overflow-x-auto">
            <table className="w-full text-[14px]">
              <thead>
                <tr style={{ background: "var(--grey-50)" }}>
                  <th className="text-left px-4 py-3 font-semibold" style={{ color: "var(--grey-600)" }}>Staff Name</th>
                  <th className="text-right px-4 py-3 font-semibold" style={{ color: "var(--grey-600)" }}>Employee CPF</th>
                  <th className="text-right px-4 py-3 font-semibold" style={{ color: "var(--grey-600)" }}>Employer CPF</th>
                  <th className="text-right px-4 py-3 font-semibold" style={{ color: "var(--grey-600)" }}>Total CPF</th>
                </tr>
              </thead>
              <tbody>
                {staffSummary.map(([userId, data]) => (
                  <tr key={userId} style={{ borderBottom: "1px solid var(--grey-200)" }}>
                    ...
                    <td className="px-4 py-3 text-right tabular-nums">{formatCurrency(data.cpfEmployer)}</td>
                    <td className="px-4 py-3 text-right tabular-nums font-bold" style={{ color: "var(--blue-500)" }}>
                      {formatCurrency(data.cpfEmployee + data.cpfEmployer)}
                    </td>
                  </tr>
                ))}
                {/* Totals */}
                <tr style={{ borderTop: "2px solid var(--grey-400)", background: "var(--grey-50)" }}>
                  ...
                  <td className="px-4 py-3 text-right font-bold tabular-nums">
                    {formatCurrency(staffSummary.reduce((s, [, d]) => s + d.cpfEmployee, 0))}
                  </td>
                  <td className="px-4 py-3 text-right font-bold tabular-nums">
                    {formatCurrency(staffSummary.reduce((s, [, d]) => s + d.cpfEmployer, 0))}
                  </td>
                  <td className="px-4 py-3 text-right font-bold tabular-nums" style={{ color: "var(--blue-500)" }}>
                    {formatCurrency(staffSummary.reduce((s, [, d]) => s + d.cpfEmployee + d.cpfEmployer, 0))}
                  </td>
                </tr>
              </tbody>
            </table>
