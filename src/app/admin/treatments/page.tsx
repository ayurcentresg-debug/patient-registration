      ...
      <div className="fixed z-[151] yoda-slide-in" style={{ top: "50%", left: "50%", transform: "translate(-50%, -50%)", width: 520, maxHeight: "90vh", overflow: "auto", background: "var(--white)", border: "1px solid var(--grey-300)", borderRadius: "var(--radius)", boxShadow: "0 12px 40px rgba(0,0,0,0.2)" }}>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button type="submit" disabled={saving} className="px-5 py-2 text-[13px] font-bold text-white rounded" style={{ background: saving ? "var(--grey-400)" : "#2d6a4f" }}>
    ...
    <div className="p-6 md:p-8 yoda-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => { setEditTreatment(null); setShowTreatmentModal(true); }} className="inline-flex items-center gap-2 text-white px-5 py-2 text-[13px] font-semibold rounded" style={{ background: "#2d6a4f", borderRadius: "var(--radius-sm)" }}>
              ...
              <div key={t.id} style={{ ...cardStyle, opacity: t.isActive ? 1 : 0.6 }}>
                <div className="flex items-center gap-2 px-4 py-2.5">
                  ...
                  <div className="flex-shrink-0" style={{ width: 200 }}>
                    <span className="text-[13px] font-bold block truncate" style={{ color: "var(--grey-900)" }}>{t.name}</span>
                    <span className="text-[10px]" style={{ color: "var(--grey-500)" }}>{t.duration} min{!t.isActive ? " · Inactive" : ""}</span>
                  </div>
                  {/* Package chips inline */}
                  <div className="flex-1 flex items-center gap-1 min-w-0 overflow-x-auto" style={{ scrollbarWidth: "none" }}>
                    {t.packages.map(pkg => (
                      <div key={pkg.id} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full cursor-pointer hover:shadow-sm flex-shrink-0" onClick={() => { setPackageForTreatment(t); setEditPackage(pkg); setShowPackageModal(true); }} style={{ background: pkg.isActive ? "#f0fdf4" : "var(--grey-50)", border: `1px solid ${pkg.isActive ? "#a7e3bd" : "var(--grey-200)"}`, opacity: pkg.isActive ? 1 : 0.5 }}>
                        <span className="text-[10px] font-semibold" style={{ color: "var(--grey-600)" }}>{pkg.sessionCount}x</span>
                        <span className="text-[10px] font-bold" style={{ color: "var(--grey-900)" }}>{formatCurrency(pkg.pricePerSession)}/s</span>
                        <span className="text-[8px] font-bold px-1 rounded-full" style={{ background: "#c8e6c9", color: "#2e7d32" }}>{pkg.discountPercent}%</span>
                      </div>
                    ))}
                    <button onClick={() => { setPackageForTreatment(t); setEditPackage(null); setShowPackageModal(true); }} className="inline-flex items-center justify-center w-5 h-5 rounded-full transition-colors hover:bg-gray-100 flex-shrink-0" style={{ border: "1px dashed var(--grey-300)", color: "#2d6a4f" }} title="Add package">
                  ...
                  {/* Price */}
                  <span className="text-[14px] font-bold flex-shrink-0" style={{ color: "var(--grey-900)" }}>{formatCurrency(t.basePrice)}</span>
                  {/* Actions */}
                  <div className="flex items-center gap-0.5 flex-shrink-0">
                    <button onClick={() => { setEditTreatment(t); setShowTreatmentModal(true); }} className="w-7 h-7 flex items-center justify-center rounded hover:bg-gray-100" title="Edit">
                    ...
                    <button onClick={() => handleDeleteTreatment(t.id, t.name)} className="w-7 h-7 flex items-center justify-center rounded hover:bg-gray-100" title="Delete">
